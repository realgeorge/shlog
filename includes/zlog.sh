...
then
	...
	LOG_TRACE_COLOR="$(tput bold setaf 4)"
fi
...
log() {
	...
	log_level_int="$(eval \$LOG_LEVEL_${log_level})";
	log_trace_int="$(eval \$LOG_LEVEL_TRACE_${log_level})"
	log_level_stdout="$(eval \$LOG_LEVEL_${LOG_LEVEL_STDOUT})"
	#eval log_color="\$log_color" # TODO: Remove this, it's not necessary colors are defined globally

	case $LOG_FORMAT_PRESET in
		symmetric) log_prefix=$( [ "$log_level_int" -lt "$log_trace_int" ] && echo "{$LOG_TRACE_COLOR} [TRACE] " || expr "[$log_level]" : '\(.\{0,10\}\)') ;;
		standard)  log_prefix="[$log_level]" ;;
		tagged)	   log_prefix="$log_level:" ;;
		*)         echo "Unknown style: $LOG_FORMAT_PRESET (use: symmetric|simple|classic)" ;;
	esac

	if [ $log_level_stdout -le $log_level_int ]; then
		# STDOUT
		printf "${log_color}[$(date +"%Y-%m-%d %H:%M:%S")] ${log_prefix} ${log_text} ${LOG_DEFAULT_COLOR}\n";
	fi
	...
	if [ $log_level_log -le $log_level_int ]; then
		...
		if [ ! -z $LOG_PATH ]; then
			printf "$[$(date +"%Y-%m-%d %H:%M:%S")] ${log_prefix} ${log_text}\n" >> $LOG_PATH;
		fi
	else
		if [ ! -z $LOG_PATH ]; then
			printf "[$(date +"%Y-%m-%d %H:%M:%S")] ${log_prefix} < ${FUNC}\n" >> $LOG_PATH;
		fi
	fi
	return 0;
}
...
case "$SCRIPT_EXTENSION" in
	...
	zsh)
		...
		SCRIPTENTRY() {
			local msg=$1
			log "> ${FUNCNAME[1]}" "TRACE" "${LOG_TRACE_COLOR}"
		}
