...
log() {
	unset log_args log_level log_label log_text
	unset log_label_color log_text_color log_badopt
	unset t_flag l_flag dmesg
		...
		case "$1" in
			-t|-T|-l|-L)
				if is_int "$2" && ! in_range "$2"; then
					...
					log_add_args "$1"; t_flag=1
				elif [ -z "$label_flag" ]; then 
					log_add_args "$1"; l_flag=1
				else 
	...
	if [ "$log_level_stdout" -le "$log_level_int" ]; then
		# STDOUT
		printf "${LOG_LABEL_COLOR}[%s] %s${LOG_DEFAULT_COLOR} ${LOG_TEXT_COLOR-log_text_color}%s${LOG_DEFAULT_COLOR}\n" "$(date +"%Y-%m-%d %H:%M:%S")" "$log_prefix" "$log_text"
	fi
